...
export function GetLinksYears({ years }: any) {
   ...
   years.map((label: any) => (
      links.push({
         label: label,
         route: `/admin/dashboard/${up}/${label}/${section}`
      })
   ))
...
export function GetLinksCenters({ centros }: any) {
   ...
   centros.forEach((centro: any) => {
      if (centro.name != 'Pediatria') {
         links.push({
            label: centro.name,
            route: `/admin/dashboard/${centro.id}/${year}/${section}`
         })
      }
   });
...
export function GetSectionButtons() {
   ...
   let links: object[] = [{
      label: 'General',
      route: `/admin/dashboard/${center}/${year}/general`
   }, {
      label: 'CPR',
      route: `/admin/dashboard/${center}/${year}/cpr`
   }, {
      label: 'Sense CPR',
      route: `/admin/dashboard/${center}/${year}/nocpr`
   }, {
      label: 'Contractes',
      route: `/admin/dashboard/${center}/${year}/contractes`
   }];
